...
def index(request): 
    ...
    for book in books:       
        if book.cover=="":
            book_cover = str('book_covers/book-media-1.jpg')
        else: 
            book_cover=str(book.cover)
        for contributor in contributors: 
            if contributor.id==book.id:
                author=contributor
                break

        all_books.append({"book": book, "book_cover":book_cover, "author":author})
...
def book_list(request):
    ...
    for book in books:
        ...
        books_with_reviews.append({"book": book, "book_rating": book_rating, \
            "number_of_reviews": number_of_reviews, "book_cover":book_cover, "author":author, "sample": book_sample})

    context = {
        "book_list": books_with_reviews
    }
    return render(request, "reviews/publications.html", context)

def book_detail(request, pk):
    book = get_object_or_404(Book, pk=pk)
    contributors=Contributor.objects.filter(id=book.contributors)
    context = {
            "book": book,
